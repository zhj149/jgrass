...
import org.jfree.experimental.chart.swt.editor.SWTChartEditorDialog;
...
public class ChartComposite extends Composite
    ...
    /**
     * Create a new chart composite with a default FillLayout. This way, when drawn, the chart will
     * fill all the space.
     * 
    ...
    public ChartComposite( Composite comp, int style ) {
        this(comp, style, null, DEFAULT_WIDTH, DEFAULT_HEIGHT, DEFAULT_MINIMUM_DRAW_WIDTH,
    ...
    public ChartComposite( Composite comp, int style, JFreeChart chart ) {
        this(comp, style, chart, DEFAULT_WIDTH, DEFAULT_HEIGHT, DEFAULT_MINIMUM_DRAW_WIDTH,
                DEFAULT_MINIMUM_DRAW_HEIGHT, DEFAULT_MAXIMUM_DRAW_WIDTH,
                DEFAULT_MAXIMUM_DRAW_HEIGHT, DEFAULT_BUFFER_USED, true, // properties
                true, // save
                true, // print
     ...
     */
    public ChartComposite( Composite comp, int style, JFreeChart chart, boolean properties,
            boolean save, boolean print, boolean zoom, boolean tooltips ) {
        this(comp, style, chart, DEFAULT_WIDTH, DEFAULT_HEIGHT, DEFAULT_MINIMUM_DRAW_WIDTH,
                DEFAULT_MINIMUM_DRAW_HEIGHT, DEFAULT_MAXIMUM_DRAW_WIDTH,
                DEFAULT_MAXIMUM_DRAW_HEIGHT, DEFAULT_BUFFER_USED, properties, save, print, zoom,
                tooltips);
    }
     ...
     * @param tooltips a flag indicating whether or not tooltips should be enabled for the chart.
     */
    public ChartComposite( Composite comp, int style, JFreeChart jfreechart, int width, int height,
    ...
    private void attemptEditChartProperties() {
        SWTChartEditorDialog editor = new SWTChartEditorDialog(this.getShell(), this.chart);
    ...
    public void restoreAutoDomainBounds() {
        ...
        if (p instanceof Zoomable) {
            ...
            org.eclipse.swt.graphics.Point zp = (this.zoomPoint != null
                    ? this.zoomPoint
     ...
     * 
     * @param flag <code>true</code> enables trace lines for the mouse pointer on the horizontal
     *        axis.
     */
     ...
     * 
     * @param flag <code>true</code> enables trace lines for the mouse pointer on the vertical
     *        axis.
     */
    ...
    public void widgetSelected( SelectionEvent e ) {
        ...
        if (command.equals(PROPERTIES_COMMAND)) {
            ...
            try {
                ...
                ChartPlugin.log("ChartPlugin problem", ex); //$NON-NLS-1$
                ex.printStackTrace();
            }
        ...
        /*
         * in the next zoomPoint.x and y replace by e.x and y for now. this helps to handle the
         * mouse events and besides, those values are unused AFAIK.
         */
    ...
    public int print( Graphics graphics, PageFormat pageFormat, int pageIndex )
        ...
        /*
         * CairoImage image = new CairoImage( this.getBounds().width, this.getBounds().height );
         * Graphics2D g2 = image.createGraphics2D(); double x = pageFormat.getImageableX(); double y =
         * pageFormat.getImageableY(); double w = pageFormat.getImageableWidth(); double h =
         * pageFormat.getImageableHeight(); this.chart.draw( g2, new Rectangle2D.Double(x, y, w, h),
         * this.anchor, null );
         */
        return PAGE_EXISTS;
    }
    ...
    /**
     * Hook an SWT listener on the canvas where the chart is drawn. The purpose of this method is to
    ...
    public void mouseMove( MouseEvent event ) {
        ...
        if (this.zoomPoint != null) {
            ...
            org.eclipse.swt.graphics.Point movingPoint = getPointInRectangle(event.x, event.y,
                    scaledDataArea);
            if (this.orientation == PlotOrientation.HORIZONTAL) {
                hZoom = this.rangeZoomable;
                vZoom = this.domainZoomable;
    ...
    public void paintControl( PaintEvent e ) {
        ...
        if (this.chart == null) {
            this.canvas.drawBackground(e.gc, available.x, available.y, available.width,
                    available.height);
            return;
        }
        ...
        if (this.useBuffer) {
            ...
            if (this.refreshBuffer) {
                ...
                if (scale) {
                    ...
                    this.chart.draw(sg2d, new Rectangle2D.Double(0, 0, drawWidth, drawHeight),
                            getAnchor(), this.info);
                } else {
                    this.chart.draw(sg2d, new Rectangle2D.Double(0, 0, drawWidth, drawHeight),
                            getAnchor(), this.info);
        ...
        // or redrawing the chart every time...
            ...
            this.chart.draw(sg2,
                    new Rectangle2D.Double(0, 0, getBounds().width, getBounds().height),
                    getAnchor(), this.info);
        }
        ...
        if (this.verticalAxisTrace && area.x < this.verticalTraceLineX
            ...
            e.gc.drawLine(this.verticalTraceLineX, area.y, this.verticalTraceLineX, area.y
                    + area.height);
        }
