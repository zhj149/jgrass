...
public class IUHDiffusion implements IUHCalculator {
     ...
     * @param out
     */
    public IUHDiffusion( EffectsBox effectsBox, ParameterBox fixedParams, PrintStream out ) {
        ...
        IUHDiffusionSurface iuhDiffSurface = new IUHDiffusionSurface(ampi_super, fixedParams, out);
        ...
        for( int tp = 1; tp <= tcorr; tp += timestep ) {
            ...
            dt = FluidUtils.henderson(totalampidiffusion, tp);
            tstar = tp + dt;
            if (tstar < tcorr) {
                prov = n_idf
                        - 1
                        + (tp
                                * (double) FluidUtils.width_interpolate(totalampidiffusion, tstar,
                                        0, 1) / (area_tot * ((double) FluidUtils.width_interpolate(
                                totalampidiffusion, tstar, 0, 2) - (double) FluidUtils
    ...
    private double[][] calculateTotalDiffusion( double[][] ampidiffsurface,
        ...
        if (ampisubsurface == null) {
            ...
            for( int i = 0; i < ampidiffsurface.length; i++ ) {
                ...
                if (isNovalue(intsub)) {
                    out
                            .println("Found undefined interpolated value for subsuperficial. Not summing it. Index: "
                                    + i);
