...
/**
 * This reads the fluidturtle ascii raster format. This will be like the following: <i>index {3}
 * </i><b> which defines the number of following blocks</b> <i>1: float array dem header
 * {10,10,5110830.0,1637140} </i><b> first block with resolutions and south and north coordinate</b>
 * <i>2: float array novalue {-1,-9999.0}</i><b> second block which holds novalues. In Jgrass the
 * second will be considered</b> <i>3: float matrix dem {361,355}</i><b> third block containing
 * the rows and columns of the data matrix</b> <i>the data matrix </i>
 * 
 * @author Andrea Antonello - www.hydrologis.com
 */
public class FluidturtleAsciiRasterReader extends MapReader {
    ...
    public FluidturtleAsciiRasterReader() {
    ...
    private boolean getRasterMapTypesAndData() {
        ...
        try {
            //
            BufferedReader fluidasciireader = new BufferedReader(new FileReader(theFilePath));
            // read the header
                                ...
                                if ((line = fluidasciireader.readLine().toLowerCase().trim()) != null) {
                                    if (line.trim().startsWith((i + 1) + ":")) {
                                        ...
                                        // defines the novalue
                                        {
                                            ...
                                            novalue = new Double(Double.parseDouble(noValues[1]
                                                    .trim()));
                                            break;
                                        ...
                                        // defines the data matrix rows and cols
                                        {
                                            ...
                                            fileMapHeader.put("rows", rowsandcols[0].trim());
                                            fileMapHeader.put("cols", rowsandcols[1].trim());
                                            break;
                                        } else {
                                            ...
                                            fileMapHeader.put("e-w res", values[0].trim());
                                            fileMapHeader.put("n-s res", values[1].trim());
                                            fileMapHeader.put("south", values[2].trim());
                                            fileMapHeader.put("west", values[3].trim());
                                            break;
                                        }
            ...
            if (fileMapHeader.containsKey("n-s res")) {
                fileWindow = new JGrassRegion(Double.parseDouble(fileMapHeader.get("west")),
                        east, Double.parseDouble(fileMapHeader.get("south")), north,
                        Double.parseDouble(fileMapHeader.get("e-w res")), Double
                                .parseDouble(fileMapHeader.get("n-s res")));
            } else if (fileMapHeader.containsKey("cols")) {
                fileWindow = new JGrassRegion(Double.parseDouble(fileMapHeader.get("west")),
                        east, Double.parseDouble(fileMapHeader.get("south")), north,
                        Integer.parseInt(fileMapHeader.get("rows")), Integer
                                .parseInt(fileMapHeader.get("cols")));
            }
            ...
            for( int i = 0; i < fileWindow.getRows(); i++ ) {
                // if the line is empty, just jump it
                if (line.length() < 1 || line.equals("") || line.equals(" ")) {
                    i--;
                    if ((line = fluidasciireader.readLine()) != null) {
                        continue;
                    } else {
                ...
                for( int j = 0; j < fileWindow.getCols(); j++ ) {
                    ...
                    if (!value.equals(String.valueOf(novalue))) {
    ...
    private boolean readRasterRow( int currentfilerow, byte[] rowDataCache, byte[] rowColorDataCache )
        ...
        for( int col = 0; col < dataWindow.getCols(); col++ ) {
            ...
            if ((currentfilecol < 0 || currentfilecol >= fileWindow.getCols())) {
                ...
                /*
                 * rowCache.position(currentfilecol * numberOfBytesPerValue); double cell =
                 * rowCache.getDouble();
                 */
