...
public class CompressesRasterWriter {
     ...
     * @param _dataWindow
     */
    public CompressesRasterWriter( int _outputToDiskType, boolean _jump, double[] _range,
            long _pointerInFilePosition, long[] _rowaddresses, JGrassRegion _dataWindow ) {
        ...
        try {
            ...
            for( int i = 0; i < dataObject.getRows(); i++ ) {
                for( int j = 0; j < dataObject.getCols(); j++ ) {
                    if (jump)
                        break;
                    double value = dataObject.getValueAt(i, j);
                    if (!isNovalue(value)) {
