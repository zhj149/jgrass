...
/**
 * This reads the esri ascii raster grid
 * 
 * @author Andrea Antonello - www.hydrologis.com
 */
public class EsriAsciiRasterReader extends MapReader {
    ...
    private boolean getRasterMapTypesAndData() {
        ...
        try {
            //
            BufferedReader esriasciireader = new BufferedReader(new FileReader(theFilePath));
            // read the header
            ...
            while( !readheader ) {
                if ((line = esriasciireader.readLine().toLowerCase().trim()) != null) {
                    ...
                    if (tok.countTokens() == 2) {
                        ...
                        if (key.startsWith("xll")) {
                            key = "xllcorner";
                        }
                        if (key.startsWith("yll")) {
                            key = "yllcorner";
                        }
                        ...
                        if (key.startsWith("dim") || key.startsWith("cellsize")) {
                            key = "cellsize";
                        }
                        if (key.startsWith("nov") || key.startsWith("nodata")) {
                            key = "nodata_value";
                        }
            ...
            south = Double.parseDouble(fileMapHeader.get("yllcorner"));
            west = Double.parseDouble(fileMapHeader.get("xllcorner"));
            thecols = Integer.parseInt(fileMapHeader.get("ncols"));
            therows = Integer.parseInt(fileMapHeader.get("nrows"));
            xres = yres = Double.parseDouble(fileMapHeader.get("cellsize"));
            novalue = new Double(Double.parseDouble(fileMapHeader.get("nodata_value")));
