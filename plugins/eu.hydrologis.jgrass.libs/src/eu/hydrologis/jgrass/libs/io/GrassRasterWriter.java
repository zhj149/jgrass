...
public class GrassRasterWriter extends MapWriter {
    ...
    public boolean open( String fileName, String locationPath, String mapsetName ) {
        ...
        if (dataWindow != null) {
            ...
            fcellFilePath = mapsetPath + File.separator + JGrassConstants.FCELL + File.separator
                    + name;
    ...
    public boolean write( Object dataObject ) throws Exception {
        ...
        File ds2 = new File(mapsetPath + File.separator + JGrassConstants.CELL_MISC
                + File.separator + name + File.separator + JGrassConstants.CELLMISC_NULL);
        RandomAccessFile theCreatedNullFile = new RandomAccessFile(ds2, "rw"); //$NON-NLS-1$

        /*
         * finally writing to disk
         */
        CompressesRasterWriter crwriter = new CompressesRasterWriter(outputToDiskType, jump, range,
    ...
    private boolean createEmptyHeader( String filePath, int rows ) {
        ...
        try {
            ...
            JGrassLibsPlugin
                    .log(
                            "JGrassLibsPlugin problem: eu.hydrologis.jgrass.libs.io#GrassRasterWriter#createEmptyHeader", e); //$NON-NLS-1$
            e.printStackTrace();
            return false;
        }
    ...
    private void createUtilityFiles() throws Exception {
        ...
        // cell/<name>
        ds = new File(mapsetPath + File.separator + JGrassConstants.CELL + File.separator + name);
        ...
        // f_format
        ds = new File(mapsetPath + File.separator + JGrassConstants.CELL_MISC + File.separator
                + name + File.separator + JGrassConstants.CELLMISC_FORMAT);
        ...
        // f_quant
        ds = new File(mapsetPath + File.separator + JGrassConstants.CELL_MISC + File.separator
                + name + File.separator + JGrassConstants.CELLMISC_QUANT);
        ...
        // f_range
        ds = new File(mapsetPath + File.separator + JGrassConstants.CELL_MISC + File.separator
                + name + File.separator + JGrassConstants.CELLMISC_RANGE);
         ...
         */
        JGrassRegion tmp = new JGrassRegion(locationPath + File.separator
                + JGrassConstants.PERMANENT_MAPSET + File.separator + JGrassConstants.WIND);
        createCellhd(tmp.getProj(), tmp.getZone(), dataWindow.getNorth(), dataWindow.getSouth(),
                dataWindow.getEast(), dataWindow.getWest(), dataWindow.getCols(), dataWindow
                        .getRows(), dataWindow.getNSResolution(), dataWindow.getWEResolution(), -1,
                1);

        // hist/<name>
        ds = new File(mapsetPath + File.separator + JGrassConstants.HIST + File.separator + name);
        ...
        // create a default colortable
        List<String> colorRules = JGrassColorTable.createDefaultColorTable(range, 255);
        File colrFile = new File(mapsetPath + File.separator + JGrassConstants.COLR
                + File.separator + name);
        BufferedWriter bw = new BufferedWriter(new FileWriter(colrFile));
        if (colorRules.size() != 0) {
            String header = "% " + range[0] + "   " + range[1] + "   " + 255;
            bw.write(header + "\n");
            for( String r : colorRules ) {
                bw.write(r + "\n");
            }
            bw.close();
        }
    ...
    private void createCellhd( int chproj, int chzone, double chn, double chs, double che,
            ...
            int chcompressed ) throws Exception {
        ...
        File ds = new File(mapsetPath + File.separator + JGrassConstants.CELLHD + File.separator
                + name);
        OutputStreamWriter windFile = new OutputStreamWriter(new FileOutputStream(ds));
