package eu.hydrologis.jgrass.libs.map.color;
...
public class PredefinedColorRules {
    ...
    public static HashMap<String, String[][]> getColorsFolder( boolean doReset ) {
        ...
        try {
            ...
            Bundle bundle = Platform.getBundle(JGrassLibsPlugin.PLUGIN_ID);
            if (bundle != null) {
            ...
            if (colorTablesFolder != null && colorTablesFolder.exists()) {
                ...
                for( File file : files ) {
                    ...
                    if (name.toLowerCase().endsWith(".clr")) {
                        ...
                        String ruleName = FileUtilities.getNameWithoutExtention(file);
    ...
    public static List<RuleValues> getRulesValuesList( String[][] colorRules, double[] dataRange )
            throws IOException {
        ...
        if (colorRules[0].length == 3) {
            ...
            for( int i = 0; i < colorRules.length - 1; i++ ) {
                try {
                    ...
                    Color fromColor = new Color(Integer.parseInt(colorRules[i][0]), Integer
                            .parseInt(colorRules[i][1]), Integer.parseInt(colorRules[i][2]));
                    double to = dataRange[0] + ((i + 1) * rinc);
                    Color toColor = new Color(Integer.parseInt(colorRules[i + 1][0]), Integer
                            .parseInt(colorRules[i + 1][1]), Integer.parseInt(colorRules[i + 1][2]));
            ...
            for( int i = 0; i < colorRules.length; i++ ) {
                try {
                    ...
                    Color fromColor = new Color(Integer.parseInt(colorRules[i][1]), Integer
                            .parseInt(colorRules[i][2]), Integer.parseInt(colorRules[i][3]));
                    double to = Double.parseDouble(colorRules[i][4]);
                    Color toColor = new Color(Integer.parseInt(colorRules[i][5]), Integer
                            .parseInt(colorRules[i][6]), Integer.parseInt(colorRules[i][7]));
                    RuleValues rV = new RuleValues();
                    rV.fromValue = from;
                    rV.toValue = to;
                    rV.fromColor = fromColor;
