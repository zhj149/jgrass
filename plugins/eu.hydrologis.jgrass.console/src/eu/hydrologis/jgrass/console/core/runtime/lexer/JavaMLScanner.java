...
import eu.hydrologis.jgrass.console.core.runtime.nodes.X_argument;
import eu.hydrologis.jgrass.console.core.runtime.nodes.X_assign;
import eu.hydrologis.jgrass.console.core.runtime.nodes.X_asterisk;
...
import eu.hydrologis.jgrass.console.core.runtime.nodes.X_directive_usage;
import eu.hydrologis.jgrass.console.core.runtime.nodes.X_input;
...
import eu.hydrologis.jgrass.console.core.runtime.nodes.X_output;
...
/**
 * <p>This scanner scans <i>JGRASS</i> Java based, OpenMI compliant models.</p>
 * @see eu.hydrologis.jgrass.console.core.runtime.parser.JavaMLParser
 ...
 */
public final class JavaMLScanner
	extends AbstractLexer<TOKs>
	implements Lexer<TOKs> {

// Construction
	/**
	 * <p>The copy constructor <code>JavaMLScanner</code> creates this
	 ...
	 */
	public JavaMLScanner( Projectspace projectSpace, Reader sourceCode,
			int line ) {
...
// Operations
	...
	public Lexeme<TOKs>[] lexemes() {
		...
		try {
			...
			lexemes.add(
					new X_directive_usage(
							projectSpace().languageBundle().getString(
									"HgisML.usage" //$NON-NLS-1$
								)
						)
				);
			lexemes.add(
					new X_asterisk(
							projectSpace().languageBundle().getString(
									"HgisML.operator_asterisk" //$NON-NLS-1$
								)
						)
				);
			
			// Adding the java model keywords first...
			Vector<String> keywords = projectSpace().importedJavaModelKeywords();
			...
			// Adding the native model keywords...
			keywords = projectSpace().importedNativeModelKeywords();
			for( int i = 0; i < keywords.size(); ++i ) {
			...
			lexemes.add(
					new X_argument(
							projectSpace().languageBundle().getString(
									"HgisML.argument_flag" //$NON-NLS-1$
								)
						)
				);
			lexemes.add(
